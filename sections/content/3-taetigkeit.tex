\section{Tätigkeit (Schenkel \& Heumann)}

\subsection{Aufsetzen der Umgebung des Netzproviders (Schenkel \& Heumann)}
...
\subsection{Implementierung der Server-Client Architektur (Schenkel \& Heumann)}
\label{server-client-architektur}
	...
	Codes darunter nicht leidet.
		...
		Wenn eine Nachricht empfangen wurde, ist sie im übergebenen Array \emph{client\_message} eingetragen. Dieses
    ...
    \subsubsection{Implementierung des Clients (Schenkel)}
    ...
    Python den Kommunikationsaufbau mit Chipkarten ermöglichen (card-Utils, card-Sim und card-USIM).
        ...
        \textit{handle\_usim} definiert. Wenn externe Informationen vorhanden sind, werde sie für den Aufruf
